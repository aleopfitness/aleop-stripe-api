...
for (const tm of teamMembers) {
  const tmId = tm.id;
  const currentClub = tm?.customFields?.club;
  if (String(currentClub) === String(newClub)) {
    console.log('[MS-UPDATED] Skip (same club)', { tmId, newClub });
    continue;
  }
  console.log('[MS-UPDATED] Updating club for member', { tmId, from: currentClub, to: newClub });
  try {
